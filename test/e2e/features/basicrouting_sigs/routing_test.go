...
import (
	...
	"github.com/kgateway-dev/kgateway/v2/test/e2e/features/basicrouting_sigs/assertions"
)
...
func TestGatewayWithRoute(t *testing.T) {
	feat := features.New("Gateway with Route").
		Setup(func(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			cmd := exec.CommandContext(ctx, "kubectl", "get", "gateway", "gateway", "-n", "default", "-o", "json")
