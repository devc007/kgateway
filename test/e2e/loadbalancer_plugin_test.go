	...
	Context("Maglev LoadBalancer", func() {
		...
		It("should have expected envoy config", func() {
			Eventually(func(g Gomega) {
				...
				g.Expect(dacs[0].LbPolicy).To(Equal(clusterv3.Cluster_MAGLEV))
	...
	Context("Maglev LB w/ close connections on set change", func() {
		...
		It("should have expected envoy config", func() {
			Eventually(func(g Gomega) {
				...
				g.Expect(dacs[0].LbPolicy).To(Equal(clusterv3.Cluster_MAGLEV), dacs[0])
...
func findDynamicActiveClusters(dump *envoy_admin_v3.ConfigDump) ([]*clusterv3.Cluster, error) {
