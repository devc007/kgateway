...
func TestAddKeysetToFetcher(t *testing.T) {
	...
	f.addKeyset("https://test/jwks", 5*time.Minute, false)
...
func TestFetchJwksDiscovery(t *testing.T) {
	ctx := t.Context()
	...
	// Add keyset with discovery enabled
	f.addKeyset("https://accounts.google.com", 5*time.Minute, true)
	updates := f.SubscribeToUpdates()
	...
	// Expect call to fetch metadata
	jwksClient.EXPECT().
		FetchDiscoveryMetadata(gomock.Any(), gomock.Eq("https://accounts.google.com")).
		Return("https://www.googleapis.com/oauth2/v3/certs", nil)

	// Expect call to fetch JWKS using the discovered URI
	jwksClient.EXPECT().
		FetchJwks(gomock.Any(), gomock.Eq("https://www.googleapis.com/oauth2/v3/certs")).
		Return(expectedJwks, nil)
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		...
		case actual := <-updates:
			cache := NewJwksCache()
			assert.NoError(c, cache.LoadJwksFromStores(actual))
			// The key in the update map remains the original JwksURL (the issuer in this case),
			// because that's what the system uses as identifier.
			assert.Equal(c, jwks, cache.jwks["https://accounts.google.com"])
		default:
			assert.Fail(c, "no updates")
		}
