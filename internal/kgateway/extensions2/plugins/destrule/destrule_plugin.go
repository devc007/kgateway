...
func (d *destrulePlugin) processBackend(kctx krt.HandlerContext, ctx context.Context, ucc ir.UniqlyConnectedClient, in ir.BackendObjectIR, outCluster *clusterv3.Cluster) {
		...
		if outlier := trafficPolicy.GetOutlierDetection(); outlier != nil {
			if getLocalityLbSetting(trafficPolicy) != nil {
				if outCluster.GetCommonLbConfig() == nil {
					outCluster.CommonLbConfig = &clusterv3.Cluster_CommonLbConfig{}
				}
				outCluster.GetCommonLbConfig().LocalityConfigSpecifier = &clusterv3.Cluster_CommonLbConfig_LocalityWeightedLbConfig_{
			...
			if tcpSettings := trafficPolicy.GetConnectionPool().GetTcp(); tcpSettings != nil {
				if tcpKeepalive := tcpSettings.GetTcpKeepalive(); tcpKeepalive != nil {
					if outCluster.GetUpstreamConnectionOptions() == nil {
						outCluster.UpstreamConnectionOptions = &clusterv3.UpstreamConnectionOptions{}
					}
					if outCluster.GetUpstreamConnectionOptions().GetTcpKeepalive() == nil {
						outCluster.GetUpstreamConnectionOptions().TcpKeepalive = &corev3.TcpKeepalive{}
					}
