...
type GlobalRateLimitIR struct {
	...
	rateLimitActions []*envoy_config_route_v3.RateLimit
}
...
func createRateLimitActions(descriptors []v1alpha1.RateLimitDescriptor) ([]*envoy_config_route_v3.RateLimit_Action, error) {
	if len(descriptors) == 0 {
		return nil, errors.New("at least one descriptor is required for global rate limiting")
	}

	var result []*envoy_config_route_v3.RateLimit_Action
	...
	for _, descriptor := range descriptors {
		...
		var actions []*envoy_config_route_v3.RateLimit_Action
		...
		for _, entry := range descriptor.Entries {
			...
			case v1alpha1.RateLimitDescriptorEntryTypeRemoteAddress:
				action.ActionSpecifier = &envoy_config_route_v3.RateLimit_Action_RemoteAddress_{
