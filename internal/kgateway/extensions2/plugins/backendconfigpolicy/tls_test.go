...
func TestTranslateTLSConfig(t *testing.T) {
	tests := []struct {
		...
		secret    *ir.Secret
		wantErr   bool
		check     func(t *testing.T, result *tlsv3.UpstreamTlsContext)
	}{
		{
		...
		},
		{
			...
			check: func(t *testing.T, result *tlsv3.UpstreamTlsContext) {
				...
				assert.Equal(t, tlsv3.TlsParameters_TLSv1_2, result.CommonTlsContext.TlsParams.TlsMinimumProtocolVersion)
				assert.Equal(t, tlsv3.TlsParameters_TLSv1_3, result.CommonTlsContext.TlsParams.TlsMaximumProtocolVersion)
		...
		},
		{
			...
			check: func(t *testing.T, result *tlsv3.UpstreamTlsContext) {
				...
				assert.Equal(t, tlsv3.SubjectAltNameMatcher_DNS, san1.SanType)
				assert.Equal(t, "test.example.com", san1.Matcher.GetExact())

				san2 := validationCtx.MatchTypedSubjectAltNames[1]
				assert.Equal(t, tlsv3.SubjectAltNameMatcher_DNS, san2.SanType)
...
func validateCommonTlsContextInline(t *testing.T, result *tlsv3.UpstreamTlsContext) {
	assert.NotNil(t, result)
	assert.NotNil(t, result.CommonTlsContext)
	assert.Len(t, result.CommonTlsContext.TlsCertificates, 1)
	validationCtx := result.CommonTlsContext.GetValidationContext()
...
func validateCommonTlsContextFiles(t *testing.T, result *tlsv3.UpstreamTlsContext) {
	assert.NotNil(t, result)
	assert.NotNil(t, result.CommonTlsContext)
	assert.Len(t, result.CommonTlsContext.TlsCertificates, 1)
	validationCtx := result.CommonTlsContext.GetValidationContext()
...
func TestVerifySanListToTypedMatchSanList(t *testing.T) {
	tests := []struct {
		name     string
		sanList  []string
		expected []*tlsv3.SubjectAltNameMatcher
	}{
		{
			name:     "empty SAN list",
			sanList:  []string{},
